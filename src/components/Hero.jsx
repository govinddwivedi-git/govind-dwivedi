  ...
  return (
    <section className="flex min-h-screen flex-wrap items-center w-[95%] m-auto">
      <motion.div className="w-full md:w-1/2" initial = {{ opacity: 0 , x:-50}}
        ...
        <h2 className="my-8 p-2 text-4xl font-bold md:text-5xl lg:text-[5rem] gradient-text cursor-target">
          {HERO.name}
        </h2>
        ...
        <p className="mb-8 p-2 text-xl text-slate-400 font-semibold cursor-target">{HERO.description}</p>
      </motion.div>
      <motion.div className="w-full md:w-1/2 lg:p-8" initial = {{ opacity: 0 , scale:0.8}} animate = {{ opacity: 1 , scale:1}} transition = {{duration : 1}}>
        <div className="flex justify-center">
          <motion.img 
            className="rounded-3xl mt-4 border-t-2 border-b-2 border-r-2 border-yellow-400 shadow-[0_4px_10px_rgba(234,179,8,0.8)] cursor-target " 
            src={govindImg} 
            width={550} 
            height={550} 
            alt="Govind Dwivedi" 
